...
//Блок преимуществ
document.querySelectorAll('.advantage__button').forEach((item) =>
  item.addEventListener('click', function(e){
    e.preventDefault();
    const id = e.target.getAttribute('href').replace('#','');


    document.querySelectorAll('.advantage__button').forEach(
      (child) => child.classList.remove('advantage__button_active') 
    );
    document.querySelectorAll('.advantages__text').forEach(
      (child) => child.classList.remove('advantages__text_active') 
    );
    item.classList.add('advantage__button_active');
    document.getElementById(id).classList.add('advantages__text_active');
  })
);
document.querySelector('.advantage__button').click();
